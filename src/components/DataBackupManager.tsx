...
import { Download, Upload, RefreshCw, Database, FileJson, Clock, CheckCircle2, HardDrive } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export const DataBackupManager = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      toast({
        title: "Berhasil",
        description: `Data berhasil diexport: ${backupData.customers.length} pelanggan, ${backupData.priceHistory.length} riwayat harga, ${backupData.customerPricingTiers.length} tier harga`,
      });
    } catch (error) {
  ...
  const handleConfirmImport = async (data: BackupData) => {
    ...
    try {
      ...
      toast({
        title: "Berhasil",
        description: `Data berhasil diimport: ${data.customers.length} pelanggan, ${data.priceHistory.length} riwayat harga, ${data.customerPricingTiers.length} tier harga`,
      });
